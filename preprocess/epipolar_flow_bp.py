...
def manual2cv(c2w_: torch.Tensor, keep_original_world_coordinate=False) -> torch.Tensor:
    c2w = c2w_.clone()
    if c2w_.shape[0] == 3:
        c2w = to4x4(c2w)
...
def diff_2d_epipolar_flow(Z: torch.Tensor, Z1: torch.Tensor, camera0: Cameras, camera1: Cameras, opticalflow: torch.Tensor):
    ...
    K = camera0.get_intrinsics_matrices()
    # __import__('ipdb').set_trace()

    c2w0 = manual2cv(camera0.camera_to_worlds, keep_original_world_coordinate=True)
    c2w1 = manual2cv(camera1.camera_to_worlds, keep_original_world_coordinate=True)
    y, x = camera0.get_image_coords(pixel_offset=0).float().unbind(-1)  # (h w 2)

    # backproject to 3d
    p_cam = (
        inv(K) @ torch.stack([x, y, torch.ones_like(x)], dim=-1).unsqueeze(-1) * Z.unsqueeze(-1)
    )  # (3 3) @ (h w 3 1) * (h w 1 1) -> (h w 3 1)
    p_homo = torch.cat([p_cam, torch.ones_like(p_cam[..., :1, :])], dim=-2)  # (h w 4 1)
    p3d = to4x4(inverse(c2w0)) @ p_homo  # (4 4) @ (h w 4 1) -> (h w 4 1)
    uvf = 1 / Z1.unsqueeze(-1) * K @ (to4x4(c2w1) @ p3d)[..., :3, :]  # (3 3) @ [(4 4) @ (h w 4 1)] -> (h w 3 1)
    uv = uvf[..., :2, 0]

    sceneflow = uv - torch.stack([x, y], dim=-1)
...
if __name__ == "__main__":
    ...
    for i, data in tqdm(enumerate(dataset)):
        ...
        diffdata = diff_2d_epipolar_flow(
            data["depth_image0"], data["depth_image1"], data["camera0"], data["camera1"], opticalflow
        )
        ...
        if args.save:
            visualize_flow(interflow, f"{interflow_path}/{data['image0_path'].stem}.png")
            np.save(f"{interflow_path}/{data['image1_path'].stem}.npy", interflow)

            visualize_flow(sceneflow, f"{sceneflow_path}/{data['image0_path'].stem}.png")
            np.save(f"{sceneflow_path}/{data['image1_path'].stem}.npy", sceneflow)

            visualize_flow(opticalflow, f"{opticalflow_path}/{data['image0_path'].stem}.png")
        ...
        axs[0][0].imshow(np.linalg.norm(opticalflow, axis=-1))
        axs[0][0].set_title("opticalflow")
        axs[0][1].imshow(np.linalg.norm(sceneflow, axis=-1))
        axs[0][1].set_title("sceneflow")
        axs[0][2].imshow(np.linalg.norm(interflow, axis=-1))
